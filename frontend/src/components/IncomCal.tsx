...
import { Controller, useController, Control, SubmitErrorHandler, SubmitHandler, useForm, UseControllerProps } from 'react-hook-form';
...
export let incomeCalData: IncomCalData;

const IncomCal = (props: Props) => {
    const classes = useStyle();
    const [modalStyle] = React.useState(getModalStyle());
    const [open, setOpen] = React.useState(false);
    const { register, watch, handleSubmit, formState: { errors }, control } = useForm<IncomCalData>({
        mode: 'onSubmit',
    ...
    const handleOnSubmit: SubmitHandler<IncomCalData> = (values) => {
        ...
        incomeCalFlag = true;
        incomeCalData = values;
        console.log(incomeCalFlag);
        // setCheck2('visible');
    }
